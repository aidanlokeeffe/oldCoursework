...
#  Description: Second set of Coding Bat recursion problems
...
def changeXY(st):
  if st == "":
    ...
    if st[0] == "x":
      return "y" + changeXY(st[1:])
    else:
      return st[0] + changeXY(st[1:])
...
def array6(nums, index):
  if index == len(nums):
    ...
    return True or array6(nums, index + 1)
  else:
...
def countAbc(st):
  if len(st) <= 2:
    return 0
  elif st[0:3] == "abc" or st[0:3] == "aba":
    return 1 + countAbc(st[2:])
  else:
...
def countHi2(st):
  if len(st) < 2:
    return 0
  elif st[1:3] == "hi" and st[0] == "x":
    return countHi2(st[3:])
  elif st[0:2] == "hi":
    return 1 + countHi2(st[1:])
  else:
    return countHi2(st[1:])
...
def nestParen(st):
  if len(st) == 0:
    ...
    return True and nestParen(st[1:len(st) - 1])
  else:
    return False
...
def strCount(st, sub):
  if len(st) < len(sub):
    return 0
  elif st[0:len(sub)] == sub:
    return  1 + strCount(st[len(sub):], sub)
  else:
    return strCount(st[1:], sub)
...
def strCopies(st, sub, n):
  if len(st) < len(sub):
    ...
    n -= 1
    return n <= 0 or strCopies(st[1:], sub, n)
  else:
...
def strDist(st, sub):
  ...
  if a < b:
    ...
    return b
  elif st[:b] == sub and st[a - b:] == sub:
    return a
  else:
    return strDist(st[1: len(st) - 1], sub)
