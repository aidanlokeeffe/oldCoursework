...
#  Description: Third set of recursion problems
...
def groupNoAdj(start, nums, target):
  if target == 0:
    ...
    return (groupNoAdj(start + 1, nums, target) or 
...
def groupSumClump(start, nums, target):
  ...
  if start >= len(nums):
    ...
    return (groupSumClump(start + count, nums, target - nums[start] * count) or 
...
def split53Helper(start, nums, sum1, sum2):
  if start >= len(nums):
    ...
    return (split53Helper(start + 1, nums, sum1 + nums[start], sum2) or
      split53Helper(start + 1, nums, sum1, sum2 + nums[start]))
